"""ablang_jax.model - a Flax reimplementation of AbLang2's AbRep encoder.

AbLang2 (OPIG, BSD-3-Clause) is a 12-block pre-norm transformer encoder: hidden 480, 20 heads,
SwiGLU feed-forward, rotary (RoPE) positions, vocab 26. This module reimplements the architecture
exactly so the original BSD-3-Clause PyTorch weights load unchanged, giving, in one differentiable
JAX forward pass, both per-residue hidden states and per-block attention maps.

Weight port: a PyTorch Linear weight [out, in] maps to a Flax Dense kernel [in, out] (transpose).
See load_ablanx_params for the full state_dict key mapping. The JAX forward is intended to match the
reference PyTorch AbLang2 to within a small tolerance; a JAX-vs-original agreement check will be
published once the port is validated.

This is a port. The AbLang2 architecture and weights are the work of the AbLang2 authors (OPIG).
See ATTRIBUTION.md and https://github.com/oxpig/AbLang2.
"""
...
def _rope_freqs(head_dim, base=10000.0):
    # AbLang2 stores rotary_emb.freqs of length head_dim//2 (=12). inv_freq over half the head dim.
    half = head_dim // 2
    return 1.0 / (base ** (jnp.arange(0, half, dtype=jnp.float32) * 2.0 / head_dim))


def _apply_rope(x, freqs):
    """x [B,L,H,D]; INTERLEAVED RoPE (rotary_embedding_torch/lucidrains, as AbLang2 uses). freqs [D//2]."""
    L = x.shape[1]
    pos = jnp.arange(L, dtype=jnp.float32)
    ang = pos[:, None] * freqs[None, :]                      # [L, D/2]
    ang = jnp.repeat(ang, 2, axis=-1)                        # [L, D]  (each freq duplicated adjacent)
    cos = jnp.cos(ang)[None, :, None, :]                     # [1,L,1,D]
    sin = jnp.sin(ang)[None, :, None, :]
    x_even = x[..., 0::2]; x_odd = x[..., 1::2]              # rotate_half interleaved: [-x_odd, x_even]
...
class Block(nn.Module):
    ...
    def __call__(self, x, mask, freqs, return_attn=False):
        B, L, _ = x.shape
        h = nn.LayerNorm(epsilon=LN_EPS, name="pre_attn_layer_norm")(x)   # pre-norm
        ...
        # AbLang2 scales q by head_dim^-0.5 AND divides by sqrt(head_dim) in attention -> net /head_dim
        logits = jnp.einsum("blhd,bmhd->bhlm", q, k) / HEAD_DIM            # [B,H,L,L]
        logits = logits + (1.0 - mask[:, None, None, :]) * -1e9            # mask keys
        attn = jax.nn.softmax(logits, axis=-1)
        o = jnp.einsum("bhlm,bmhd->blhd", attn, v).reshape(B, L, HID)
        o = nn.Dense(HID, name="out_proj")(o)
        x = x + o
        # SwiGLU FFN: intermediate_layer[0] 480->3840; AbLang2 SwiGLU: x,gate=chunk(2); silu(gate)*x; [2] 1920->480
        h2 = nn.LayerNorm(epsilon=LN_EPS, name="final_layer_norm")(x)
        up = nn.Dense(3840, name="ffn_in")(h2)
        val, gate = jnp.split(up, 2, axis=-1)                             # first half = x(value), second = gate
        x = x + nn.Dense(HID, name="ffn_out")(jax.nn.silu(gate) * val)
        return (x, attn) if return_attn else (x, None)


class Ablanx(nn.Module):
    """AbLang2 AbRep encoder in flax. Returns hidden [B,L,480] and (optionally) attentions [N_BLOCK,B,H,L,L]."""
    @nn.compact
    def __call__(self, tokens, mask, return_attn=True):
        ...
        for i in range(N_BLOCK):
            ...
            if return_attn:
                attns.append(a)
        # AbLang2 AbRep applies a final LN after all blocks; rescoding returns THIS (last_hidden_states).
